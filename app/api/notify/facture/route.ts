/**
 * POST /api/notify/facture
 * Trigger 2: send "votre facture est disponible" email to client when a facture is created.
 */
...
export async function POST(req: Request): Promise<Response> {
  const { factureId, garageId } = (await req.json()) as { factureId: string; garageId: string };
  if (!factureId || !garageId) return Response.json({ error: 'Params manquants' }, { status: 400 });

  const { data: factureRow } = await supabase
    .from('factures')
    .select('display_ref, total_ttc, due_date, clients(id, name, email), garages(name, phone)')
    .eq('id', factureId)
  ...
  if (!clientEmail) {
    await logTrigger(garageId, 'FACTURE_NOTIFY', { resourceId: factureId, status: 'skipped', message: 'Pas d\'email client' });
  ...
  const dueDate = r.due_date ? new Date(r.due_date as string).toLocaleDateString('fr-FR') : null;
...
<h2 style="margin:0 0 16px;font-size:20px;color:#1A1A2E;">🧾 Votre facture est disponible</h2>
<p style="margin:0 0 16px;font-size:14px;color:#555;line-height:1.7;">Bonjour <strong>${clientName}</strong>,<br/><br/>
Votre facture <strong>${ref}</strong> est disponible :</p>
<div style="background:#FFF7ED;border:1px solid #FED7AA;border-radius:8px;padding:16px;margin-bottom:20px;">
<table width="100%" cellpadding="0" cellspacing="0">
<tr><td style="font-size:12px;color:#C2410C;">Référence</td><td style="font-size:12px;color:#C2410C;text-align:right;">Montant TTC</td></tr>
...
${dueDate ? `<tr><td colspan="2" style="font-size:12px;color:#92400E;padding-top:8px;">Échéance : ${dueDate}</td></tr>` : ''}
</table>
</div>
${garagePhone ? `<p style="font-size:13px;color:#555;">Pour régler ou toute question : <strong>${garagePhone}</strong></p>` : ''}
<p style="margin:16px 0 0;font-size:13px;color:#888;">Merci de votre confiance,<br/><strong style="color:#FF6B2B;">${garageName}</strong></p>
</td></tr>
<tr><td style="background:#0F1117;padding:16px 32px;text-align:center;"><p style="margin:0;font-size:11px;color:#8B8FA8;">Email envoyé par ${garageName} via MecaniGo.</p></td></tr>
  ...
  const { error } = await resend.emails.send({
    ...
    to: [clientEmail],
    subject: `🧾 Votre facture ${ref} — ${garageName}`,
    html,
  });
  ...
  await logTrigger(garageId, 'FACTURE_NOTIFY', {
